...
func TestParseTitle(t *testing.T) {
	...
	input := []struct {
		...
		//},
		{
			...
			Want: &Result{Name: "無職轉生～到了異世界就拿出真本事～", Episode: 11, Metadata: data.Metadata{
				Language: data.BIG5, Quality: data.P1080, SubType: data.Internal, Fansub: []string{"桜都字幕組"}, Type: data.Episode,
			}},
		}, {
			Title: "[NC-Raws] 無職轉生～到了異世界就拿出真本事～（僅限港澳台地區） / Mushoku Tensei - 11 [WEB-DL][1080p][AVC AAC][CHS_CHT_SRT][MKV]",
			Want: &Result{Name: "無職轉生～到了異世界就拿出真本事～", Metadata: data.Metadata{
				Language: data.GB | data.BIG5, Quality: data.P1080, SubType: data.Internal, Type: data.Episode, Fansub: []string{"NC-Raws"},
			}, Episode: 11},
		}, {
			Title: "[爱恋\\u0026漫猫字幕组][1月新番][无职转生～到了异世界就拿出真本事～][Mushoku Tensei Isekai Ittara Honki Dasu][10][1080p][MP4][GB][简中]",
			Want: &Result{Name: "无职转生～到了异世界就拿出真本事～", Metadata: data.Metadata{
				Language: data.GB, Quality: data.P1080, SubType: data.Internal, Type: data.Episode, Fansub: []string{"爱恋\\u0026漫猫字幕组"},
			}, Episode: 10},
		}, {
			Title: "【悠哈璃羽字幕社】[無職轉生～到了異世界就拿出真本事～_Mushoku Tensei][08][x264 1080p][CHT]",
			Want: &Result{Name: "無職轉生～到了異世界就拿出真本事～", Metadata: data.Metadata{
				Language: data.BIG5, Quality: data.P1080, SubType: data.Internal, Type: data.Episode, Fansub: []string{"悠哈璃羽字幕社"},
			}, Episode: 8},
		}, {
			Title: "​[c.c動漫][1月新番][無職轉生～到了異世界就拿出真本事～][10][BIG5][1080P][MP4]",
			Want: &Result{Name: "無職轉生～到了異世界就拿出真本事～", Metadata: data.Metadata{
				Language: data.BIG5, Quality: data.P1080, SubType: data.Internal, Type: data.Episode, Fansub: []string{"c.c動漫"},
			}, Episode: 10},
		}, {
			Title: "[Skymoon-Raws] 無職轉生，到了異世界就拿出真本事 / Mushoku Tensei - 10 [ViuTV][WEB-DL][1080p][AVC AAC][繁體外掛][MP4+ASSx2](正式版本)",
			Want: &Result{Name: "無職轉生，到了異世界就拿出真本事", Metadata: data.Metadata{
				Language: data.BIG5, Quality: data.P1080, SubType: data.External, Type: data.Episode, Fansub: []string{"Skymoon-Raws"},
			}, Episode: 10},
		}, {
			Title: "【喵萌奶茶屋】★01月新番★[無職轉生/Mushoku Tensei][09][720p][繁體][招募翻譯校對]",
			Want: &Result{Name: "無職轉生", Metadata: data.Metadata{
				Language: data.BIG5, Quality: data.P720, SubType: data.Internal, Type: data.Episode, Fansub: []string{"喵萌奶茶屋"},
			}, Episode: 9},
		}, {
			Title: "[Lilith-Raws] 銀河騎士傳 第九行星戰役 / Knights of Sidonia S02 [01-12][Baha][WEB-DL][1080p][AVC AAC][CHT][MP4]",
			Want: &Result{Name: "銀河騎士傳 第九行星戰役", Metadata: data.Metadata{
				Fansub: []string{"Lilith-Raws"}, Quality: data.P1080, Type: data.Full, Language: data.BIG5, SubType: data.Internal,
			}, Episode: 0},
		},
	}
	for _, i := range input {
		got, err := ps.Parse(i.Title)
		if err != nil {
			t.Error(err)
		}
